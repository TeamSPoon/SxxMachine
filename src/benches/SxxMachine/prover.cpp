...
Operation* const  prover::reg_top_0 = PredTable::Register("top",0, new SxxMachine::sxx_library::pred_top_0());
Operation* const  prover::reg_on_load_prover_0 = PredTable::Register("on_load_prover",0, new pred_on_load_prover_0());
Operation* const  prover::reg_prover_0 = PredTable::Register("prover",0, new pred_prover_0());
Operation* const  prover::reg_problem_3 = PredTable::Register("problem",3, new pred_problem_3());
Operation* const  prover::reg_implies_2 = PredTable::Register("implies",2, new pred_implies_2());
Operation* const  prover::reg_opposite_2 = PredTable::Register("opposite",2, new pred_opposite_2());
Operation* const  prover::reg_add_conjunction_3 = PredTable::Register("add_conjunction",3, new pred_add_conjunction_3());
Operation* const  prover::reg_expand_3 = PredTable::Register("expand",3, new pred_expand_3());
Operation* const  prover::reg_includes_2 = PredTable::Register("includes",2, new pred_includes_2());
Operation* const  prover::reg_extend_6 = PredTable::Register("extend",6, new pred_extend_6());
Operation* const  prover::reg_refute_1 = PredTable::Register("refute",1, new pred_refute_1());
	...
	mach->CUTB = mach->CurrentChoice;
	return static_cast<Operation*>(prover::s_write->FindProc(1));
	}
	...
	if(!((areg0)->Unify(prover::posint1,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s4,prover::s_a),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s3,prover::s_a),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Term* areg1 = local_aregs[1]->Deref();
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::posint2,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s3,prover::s_a),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s2,Data::F(prover::s4,prover::s_a),Data::F(prover::s4,prover::s_a)),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Term* areg1 = local_aregs[1]->Deref();
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::posint3,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s4,prover::s_a),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s1,Data::F(prover::s3,prover::s_to_be),Data::F(prover::s4,prover::s_to_be)),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Term* areg1 = local_aregs[1]->Deref();
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::posint4,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s2,Data::F(prover::s4,prover::s_a),Data::F(prover::s4,prover::s_a)),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s4,prover::s_a),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Term* areg1 = local_aregs[1]->Deref();
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::posint5,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s4,prover::s_a),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s1,Data::F(prover::s3,prover::s_b),Data::F(prover::s4,prover::s_a)),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Term* areg1 = local_aregs[1]->Deref();
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::posint6,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s2,Data::F(prover::s4,prover::s_a),Data::F(prover::s4,prover::s_b)),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s2,Data::F(prover::s4,prover::s_b),Data::F(prover::s4,prover::s_a)),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Term* areg1 = local_aregs[1]->Deref();
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::posint7,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s4,prover::s_a),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s1,Data::F(prover::s4,prover::s_b),Data::F(prover::s2,Data::F(prover::s3,prover::s_b),Data::F(prover::s4,prover::s_a))),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Term* areg1 = local_aregs[1]->Deref();
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::posint8,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s1,Data::F(prover::s4,prover::s_a),Data::F(prover::s1,Data::F(prover::s4,prover::s_b),Data::F(prover::s3,prover::s_c))),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s1,Data::F(prover::s4,prover::s_b),Data::F(prover::s1,Data::F(prover::s4,prover::s_a),Data::F(prover::s3,prover::s_c))),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Term* areg1 = local_aregs[1]->Deref();
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::posint9,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s1,Data::F(prover::s4,prover::s_a),Data::F(prover::s3,prover::s_b)),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s1,Data::F(prover::s2,Data::F(prover::s3,prover::s_b),Data::F(prover::s4,prover::s_c)),Data::F(prover::s1,Data::F(prover::s4,prover::s_a),Data::F(prover::s3,prover::s_c))),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Term* areg1 = local_aregs[1]->Deref();
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::posint10,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s2,Data::F(prover::s1,Data::F(prover::s4,prover::s_a),Data::F(prover::s3,prover::s_c)),Data::F(prover::s1,Data::F(prover::s4,prover::s_b),Data::F(prover::s3,prover::s_c))),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(prover::s1,Data::F(prover::s2,Data::F(prover::s4,prover::s_a),Data::F(prover::s4,prover::s_b)),Data::F(prover::s3,prover::s_c)),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(prover::s2,var1,var2),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s1,var3,var4),mach))) {
		return Prolog::Fail0;
	}
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(prover::s1,var1,var2),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s2,var3,var4),mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	local_aregs[0] = Data::F(prover::s_opposite, { var1->Deref(), var3->Deref(), Data::F(prover::s_opposite, { var2->Deref(), var4->Deref(), continuation }) });
	...
	Term* areg0 = local_aregs[0]->Deref();
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(prover::s3,var1),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s4, { var1->Deref() }),mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	local_aregs[0] = continuation;
	...
	Term* areg0 = local_aregs[0]->Deref();
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(prover::s4,var1),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(Data::F(prover::s3, { var1->Deref() }),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	if(!((areg1)->Unify(prover::s_refuted,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(prover::s_refuted,mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	local_aregs[0] = continuation;
	...
	if(!((areg1)->Unify(Data::F(prover::s_fs,var3,var4,var5,var6),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(prover::s_refuted,mach))) {
		return Prolog::Fail0;
	...
	if(!((areg2)->Unify(Data::F(prover::s_fs, { var3->Deref(), var4->Deref(), var5->Deref(), var6->Deref() }),mach))) {
		return Prolog::Fail0;
	}
	...
	if(!((areg1)->Unify(Data::F(prover::s_fs,var2,var3,var4,var5),mach))) {
		return Prolog::Fail0;
	}
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(prover::s5,var1,var2),mach))) {
		return Prolog::Fail0;
	}
	...
	if(!((areg5)->Unify(prover::s_refuted,mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = var3->Deref();
	...
	if(!((areg3)->Unify(Data::F(prover::s5, { var1->Deref(), var2->Deref() }),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg4)->Unify(var4,mach))) {
		return Prolog::Fail0;
	...
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(prover::s_refuted,mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	local_aregs[0] = continuation;
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(prover::s_fs,Data::F(prover::s5,Data::F(prover::s2,var1,var2),var3),var4,var5,var6),mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = var1->Deref();
	local_aregs[1] = var7;
