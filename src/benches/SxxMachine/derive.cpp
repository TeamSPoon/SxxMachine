...
Operation* const  derive::reg_top_0 = PredTable::Register("top",0, new SxxMachine::sxx_library::pred_top_0());
Operation* const  derive::reg_ops8_0 = PredTable::Register("ops8",0, new pred_ops8_0());
Operation* const  derive::reg_log10_0 = PredTable::Register("log10",0, new pred_log10_0());
Operation* const  derive::reg_divide10_0 = PredTable::Register("divide10",0, new pred_divide10_0());
Operation* const  derive::reg_d_3 = PredTable::Register("d",3, new pred_d_3());
	...
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(derive::s2,var1,var2),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(var3,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(derive::s2,var4,var5),mach))) {
		return Prolog::Fail0;
	}
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(derive::s3,var1,var2),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(var3,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(derive::s3,var4,var5),mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	local_aregs[0] = Data::F(derive::s_d, { var1->Deref(), var3->Deref(), var4->Deref(), Data::F(derive::s_d, { var2->Deref(), var3->Deref(), var5->Deref(), continuation }) });
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(derive::s1,var1,var2),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(var3,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(derive::s2,Data::F(derive::s1,var4,var2->Deref()),Data::F(derive::s1, { var1->Deref(), var5 })),mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	local_aregs[0] = Data::F(derive::s_d, { var1->Deref(), var3->Deref(), var4->Deref(), Data::F(derive::s_d, { var2->Deref(), var3->Deref(), var5->Deref(), continuation }) });
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(derive::s5,var1,var2),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(var3,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(derive::s5,Data::F(derive::s3,Data::F(derive::s1,var4,var2->Deref()),Data::F(derive::s1, { var1->Deref(), var5 })),Data::F(derive::s7, { var2->Deref(), derive::posint2 })),mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	local_aregs[0] = Data::F(derive::s_d, { var1->Deref(), var3->Deref(), var4->Deref(), Data::F(derive::s_d, { var2->Deref(), var3->Deref(), var5->Deref(), continuation }) });
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(derive::s7,var1,var2),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(var3,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(derive::s1,Data::F(derive::s1,var4,var2->Deref()),Data::F(derive::s7, { var1->Deref(), var5 })),mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(derive::s3,var1),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(var2,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(derive::s3,var3),mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(derive::s_exp,var1),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(var2,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(derive::s1,Data::F(derive::s_exp, { var1->Deref() }),var3),mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	local_aregs[0] = Data::F(derive::s_d, { var1->Deref(), var2->Deref(), var3->Deref(), continuation });
	...
	Var* var2 = Data::V(mach);
	Var* var1 = Data::V(mach);
	if(!((areg0)->Unify(Data::F(derive::s_log,var1),mach))) {
		return Prolog::Fail0;
	}
	if(!((areg1)->Unify(var2,mach))) {
		return Prolog::Fail0;
	}
	if(!((areg2)->Unify(Data::F(derive::s5,var3,var1->Deref()),mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	local_aregs[0] = Data::F(derive::s_d, { var1->Deref(), var2->Deref(), var3->Deref(), continuation });
	...
	if(!((areg2)->Unify(derive::posint1,mach))) {
		return Prolog::Fail0;
	}
	mach->DoCut(mach->CUTB);
	...
	if(!((areg2)->Unify(derive::posint0,mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
