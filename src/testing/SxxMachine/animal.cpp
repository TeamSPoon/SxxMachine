...
Operation* const  animal::reg_animal_1 = PredTable::Register("animal",1, new pred_animal_1());
	...
	if(!((areg0)->Unify(animal::s_goat,mach))) {
		return Prolog::Fail0;
	}
	...
	Term* continuation = local_aregs[1];
	Term* areg0 = local_aregs[0]->Deref();
	if(!((areg0)->Unify(animal::s_elephant,mach))) {
		return Prolog::Fail0;
	}
	local_aregs[0] = continuation;
	mach->CUTB = mach->CurrentChoice;
	...
	mach->CUTB = mach->CurrentChoice;
	return static_cast<Operation*>(animal::s_write->FindProc(1));
	}
}
