...
gencodeforpred(Strings,_Dir,_PINF,Pred,_H,_N,_A,AA,ClassName):- 
   must_det_l((   
	...
	% writel(['static {', initforeachcontinuation(Conts,N,A),'}',wr(nl)]),           
        writel([        			wr(nl),
        'public Operation Exec(Prolog mach) {return exec_static(mach); }',wr(nl),
      'public static Operation exec_static(Prolog mach){ ',wr(nl),
...
genclausecode(_PreDecl,Clause,Last,ClassName,N,Strings) :-
     ...
     must_det_l((				
		...
		writel([
                     ...
                     'public static Operation exec_',ClassName,'__',N,'(Prolog mach){ ',
...
constructterm(cut(_,C),Strings) :- ! , writel(['mach.HC(',constructterm(C,Strings),')']).
...
constructterm(Int,_) :- float(Int) , ! , writel(['Data.Float(',Int,')']),!.
...
constructterm(Term,Strings) :- compound(Term),!,
			...
			writel(['Data.F(',writeConst(Name,Strings),',',newargs(Args,1,Strings),')']))) .

writeConst([],_Strings) :- writel(['Const.Nil']),!.
writeConst(Name,_Strings):- \+ string(Name), symbol_to_name(Name,Other)->Name==Other,!,writel([getval(stem),'.s_',Name]),!.
...
bodycont(_String,(_ :- B),_,_) :- legacy_functor(B,call,1) , ! , writel(['Prolog.Call1']) .
bodycont(_String,(_ :- B),_,_) :- legacy_functor(B,call,2) , ! , writel(['Prolog.Call2']) .
bodycont(_String,(_ :- B),_,_) :- legacy_functor(B,cut,2) , ! , writel(['Prolog.Call1']) .
%bodycont(_String,(_ :- B),_,_) :- legacy_functor(B,cut,2) , ! , writel(['Prolog.Cut2']) .
...
varnamestr(_,N):- \+ string(N),symbol_to_name(N,O)->N==O,!,writel(['s_',N]),!.
varnamestr(M,_):- writel(['s',M]),!.

declforeachstring([],_) .
declforeachstring([N|R],M) :- MM is M + 1 , declfor1string(N,M),declforeachstring(R,MM) .
...
constructnum(Int) :- integer(Int) ,!, writel(['Data.Number(',Int,'L)']).
constructnum(Int) :- float(Int) ,writel(['Data.BigDec("',Int,'")']).
