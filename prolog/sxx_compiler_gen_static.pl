...
gencode_sss(Dir,Stem,Preds):-
 must_det_l((    
  ...
  writel(['import java.util.function.Function; class ',Stem,' /*extends CodeFile*/ {']),
...
symbol_to_name(Atom,Out):- symbol_to_name1(Atom,Mid)-> Atom\==Mid,!,atom_concat('sym_',Mid,Out).
...
symbol_to_name1((';'),'system_semicolon').
symbol_to_name1((':'),'system_colon').
symbol_to_name1(Atom,Out):- name(Atom,[C|Rest]), \+ code_type(C,alnum), C\==95,!,
  atomic_list_concat(['_c',C,'_'],Mid1),
  name(NAtom,Rest),symbol_to_name1(NAtom,Mid2),atom_concat(Mid1,Mid2,Out).
symbol_to_name1(Atom,Out):- name(Atom,[C|Rest]),name(Mid1,[C]),name(NAtom,Rest),
  symbol_to_name1(NAtom,Mid2),atom_concat(Mid1,Mid2,Out).
...
genclausecode(_PreDecl,Clause,Last,ClassName,N,Strings) :-
     ...
     must_det_l((				
		...
		writel([
                     ...
                     'public static Function<PrologMachine,Function> exec_',ClassName,'__',N,'(PrologMachine mach){ ',
			...
			nullifyaregs(Clause) ,
			'return ',bodycont(Clause,Name,Arity),' ;}',
			 wr(nl)]))).
...
constructterm(cut(_,C),Strings) :- ! ,
		must_det_l(( %getnameindex(Strings,'cut',0,I),
	               writel(['F(',getval(stem),'.s0, new HeapChoice(mach.CUTB),',
			constructterm(C,Strings),
			')']))) .

constructterm(Int,_) :- integer(Int) , ! , writel([getval(stem),'.']), posneg(Int) .
constructterm(Atom,Strings) :- string(Atom) , !,writeFunctor(Atom,Strings).
constructterm(Int,_) :- float(Int) , ! , writel(['Float(',Int,')']),!.

constructterm(Atom,Strings) :- []==(Atom) , !,writeFunctor(Atom,Strings).
constructterm(Atom,Strings) :- atom(Atom) , !,writeFunctor(Atom,Strings).
constructterm(Term,Strings) :- compound(Term),!,
			must_det_l((
                        legacy_functor(Term,Name,_) , Term =.. [_|Args],			
			writel(['F(',writeFunctor(Name,Strings),',',newargs(Args,1,Strings),')']))) .

writeFunctor([],_Strings) :- writel(['Const.Nil']),!.
writeFunctor(Name,Strings):-   getnameindex(Strings,Name,0,I),!, writel([getval(stem),'.s',I]),!.
writeFunctor(Name,_Strings) :- must_det_l((atom_string(Name,S), writel(['Const.Intern(',format('~q',[S]),')']))),!.
...
bodycont((_ :- B),Name,Arity) :- fail, legacy_functor(B,Name,Arity) , ! ,writel(['entry_code']) .
...
bodycont((_ :- B),_,_) :- legacy_functor(B,cut,2) , ! ,
			writel(['UpperPrologMachine.Cut2']) .
bodycont((_ :- B),_,_) :- legacy_functor(B,Name0,Arity) , MArity is Arity-1, symbol_to_name(Name0,Name),
			writel(['(Function<PrologMachine,Function>)pred_',Name,'_',MArity,'::exec_static']) .
bodycont((_ :- B),_,_) :- legacy_functor(B,Name0,Arity) , symbol_to_name(Name0,Name),
...
declforeachclause([_|R],N,ClassName) :-
			writel(['static Function<PrologMachine, Function> cl',
...
declforeachcontinuation([N/A|R],Name,Arity) :-
		writel([	'static Function<PrologMachine, Function> ',
			N,
			A,
			'cont ;',
			wr(nl)
...
:- comp_to(sxx_builtins,'../jsrc/bootlib').
