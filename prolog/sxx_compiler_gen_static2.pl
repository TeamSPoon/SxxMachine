...
comp(FileSpec) :- \+ exists_file(FileSpec),!,
...
comp(File0) :-
               wdmsg(comp(File0)),
                see(File0) ,    	
		InProg = [] ,
		relative_file_name(File0,'.',File),
		file_name_extension(PlStem,_,File),
		wdmsg(file_name_extension(PlStem,_,File)),			
		readprogram(PlStem,InProg,Prog) ,!,
                seen, % close(File) ,
		makename(['tcomp_src/',PlStem],Dir),
		make_directory_path(Dir),!,
		makename([Dir,'/',PlStem,'.java'],FileName),
		tell(FileName),
		gencode(Dir,PlStem,Prog),!,
		told.
     


gencode(_,_,[]) .
gencode(Dir,PlStem,[Pred|Preds]) :- 
  gencodeforpred(Dir,PlStem,Pred) , 
  gencode(Dir,PlStem,Preds) .

call_local_side_efs(public(_)) :-!. 
call_local_side_efs(Call):- call(Call).
...
readprogram(PlStem,In,Out) :-
		read(Clause) ,local_side_efs(PlStem,Clause) ,
		(Clause = end_of_file -> In = Out
		;
		 binarize(Clause,BinCl) ,
			addclausetoprogram(BinCl,In,NewIn) ,
			readprogram(PlStem,NewIn,Out)
		) .
...
gencodeforpred(Dir,PlStem,Pred) :-
			...
			makename([Dir,'/',ClassName,'.java'],FileName),
			(var(PlStem) -> tell(FileName) ; true) ,			
			gencodeforprednow(Dir,PlStem,Pred,ClassName,N,A,AA,FileName),
			(var(PlStem) -> told ; true).
			
gencodeforprednow(_Dir,PlStem,Pred,ClassName,N,A,AA,FileName):- 			
			(var(PlStem) -> tell(FileName) ; true) ,
			...
			prelude(Pred) ,
			writel(['class /* Pred */',ClassName,' extends Code',
			    wr(nl), '{ ',wr(nl),
				declforeachstring(Strings,0),
				declforeachint(AllInts),
				declforeachcontinuation(Conts,N,A),
				declforeachclause(Pred,1,ClassName),
				'static int code_arity = ',AA,';',wr(nl),
				'static Code entry_code = new ',ClassName,'();',wr(nl),
				'static void InitStatic(PrologMachine mach){ ',wr(nl),
				  initforeachcontinuation(Conts,N,A) ,'}',wr(nl),				
				'public void Init(PrologMachine mach){InitStatic(mach);}',wr(nl),
				'int Arity() { return ',AA,' ; }',wr(nl),				
				'Code Exec(PrologMachine mach){return ExecStatic(mach);}',wr(nl),				
				'public static Code exec_',ClassName,'(PrologMachine mach){return ExecStatic(mach);}',wr(nl),
				'public static Code ExecStatic(PrologMachine mach) {',wr(nl),
				'Term aregs[] = {',aregarray(AA) ,'} ;',wr(nl),
				'mach.CreateChoicePoint(aregs) ;',wr(nl),
				'return ',ClassName,'_1(mach) ;',
				wr(nl),'}',wr(nl),'}',
				wr(nl) , wr(nl)
				]) ,!,
...
genclausecode(Clause,Last,ClassName,N,Strings) :-
		...
		writel(['/* helper */class ',ClassName,'__',N,' extends ',ClassName,wr(nl),
			'{',wr(nl),
'Code Exec(PrologMachine mach){return ExecStatic(mach);}',wr(nl),				
'public static Code exec_',ClassName,'__',N,'(PrologMachine mach){return ExecStatic(mach);}',wr(nl),
'public static Code ExecStatic(PrologMachine mach)',wr(nl),
		'{ ',
			(Last = last -> write('mach.RemoveChoice() ;')
				;
				(M is N + 1 , writel(['mach.FillAlternative(cl',M,') ;']))
			) ,wr(nl),
			...
			nullifyaregs(Clause) ,
			'return ',bodycont(Clause,Name,Arity) ,' ;',wr(nl),
	'}',wr(nl),
   '}',wr(nl), wr(nl)]) .
...
initforeachcontinuation([cut/2|R],N,A) :- !,	% Cut2
		initforeachcontinuation(R,N,A) .
initforeachcontinuation([N/A|R],Name,Arity) :- use_entry_code(N,B),!,
		B is A - 1 ,
		writel([N,A,'cont = pred_',N,'_',B,'.entry_code ;',wr(nl)]) ,
		initforeachcontinuation(R,Name,Arity) .
initforeachcontinuation([N/A|R],Name,Arity) :- use_static_call(N,B),!,
		B is A - 1 ,
	       writel([N,A,'cont = pred_',N,'_',B,'.call_pred_',N,'_',B,'.ExecStatic;',wr(nl)]) ,
		initforeachcontinuation(R,Name,Arity) .
