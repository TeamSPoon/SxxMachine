...
genclausecode(Clause,Last,ClassName,N,Strings) :-
		...
		writel(['/* */class ',ClassName,'__',N,' extends ',ClassName,wr(nl),
			'{',wr(nl),
...
declvars(1) :- ! .
declvars(N) :- M is N - 1 ,
		writel(['Term var',M,' = Term.makeVar(mach) ;',wr(nl)]) ,
		declvars(M) .
...
constructterm(cut(_,C),Strings) :- ! ,
		...
		writel(['Term.makeCompound(string',I,', new HeapChoice(mach.CUTB),',
...
declforeachint([]) .
declforeachint([N|R]) :- writel(['static IntegerTerm ',posneg(N),
				' = Term.makeInt(',N,') ;',wr(nl)]) ,
				declforeachint(R) .
